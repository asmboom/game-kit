...
The packages must be imported directly into your projects Assets folder, not in a 
sub-folder.
This means you need to move everything within the package folder 'ORK Okashi RPG Kit' 
directly into the Project tab, so that they aren't placed in a sub-folder.
...
Simply import the latest scripts to get the demo running.
...
Resources
Contains ORK's resources folder structure.
...
RPG Kit
Contains ORKs script files for Unity 4 in C#.
...
Version 1.2.7
- new: Game Events: Regenerate step - revive dead party members option.
- new: Game Events: Check status effect step - check if a status effect is or isn't applied on a player character or the whole party.
- fix: Game Settings: Game Over settings - disabling the retry, load or exit button wasn't saved.
- fix: Game Over: Using 'Retry' didn't do anything.
- fix: Load Game: Loading the game didn't set the use level of skills.
- fix: Load Game: Game/number variables have been loaded too late in some cases.
- fix: Global Events: Calling a global event from an item didn't display dialogues in some cases (the 'Accept' key from selecting the item closed the dialogue immediately).

Version 1.2.6
- new: Dialogue Positions: Box anchor setting.
- fix: Menu System: Character info menu part - back button is controllable if whole party is displayed.
- fix: Battle System: Turn Based: Combatants who died from status effects at the start of the turn will now play their death animation immediately.
- fix: Battle System: Battle actions wont perform if they're prevented by status effect blocks (e.g. when the action is already chosen but hasn't started yet).
- fix: Animations: Some idle battle animation setups could lead to stop playing the idle animation.
- fix: Dialogues: The portrait of a speaker could still be displayed in other dialogues (e.g. item collection).
