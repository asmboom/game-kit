		...
		else
		{
			...
			if(DataHolder.DialoguePositions().GetDataCount() > 0)
			{
				...
				if(fold1)
				{
					...
					DataHolder.DialoguePosition(selection).boxAnchor = (TextAnchor)EditorGUILayout.EnumPopup("Anchor", DataHolder.DialoguePosition(selection).boxAnchor, GUILayout.Width(this.mWidth));
