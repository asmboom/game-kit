...
public class GameEventTabs
{
	...
	public static void ShowTab(int index, GameEvent gameEvent, EventStep baseStep)
	{
		...
		else if(baseStep is CheckStatusEffectStep) GameEventTabs.CheckStatusEffectStep(index, gameEvent, (CheckStatusEffectStep)baseStep);
	...
	public static void LearnSkillStep(int index, GameEvent gameEvent, LearnSkillStep baseStep)
	{
		...
		if(baseStep.fold)
		{
			...
			EditorGUILayout.BeginHorizontal();
			baseStep.show = EditorGUILayout.Toggle("Level", baseStep.show, GUILayout.Width(mWidth));
			if(baseStep.show)
			{
				baseStep.number = EditorGUILayout.IntField(baseStep.number, GUILayout.Width(mWidth*0.5f));
				if(baseStep.number < 1) baseStep.number = 1;
			}
			GUILayout.FlexibleSpace();
			EditorGUILayout.EndHorizontal();
			EditorGUILayout.Separator();
	...
	public static void HasSkillStep(int index, GameEvent gameEvent, HasSkillStep baseStep)
	{
		...
		if(baseStep.fold)
		{
			...
			baseStep.show4 = EditorGUILayout.Toggle("Level", baseStep.show4, GUILayout.Width(mWidth));
			if(baseStep.show4)
			{
				baseStep.number = EditorGUILayout.IntField(baseStep.number, GUILayout.Width(mWidth*0.5f));
				if(baseStep.number < 1) baseStep.number = 1;
	...
	public static void CheckStatusEffectStep(int index, GameEvent gameEvent, CheckStatusEffectStep baseStep)
	{
		baseStep.fold = EditorGUILayout.Foldout(baseStep.fold, "Step "+index+": Check Status Effect");
		if(baseStep.fold)
		{
			GameEventTabs.EventStep(index, gameEvent, baseStep);
			EditorGUILayout.BeginHorizontal();
			...
			baseStep.number = EditorGUILayout.Popup("Status Effect", baseStep.number, DataHolder.Effects().GetNameList(true), GUILayout.Width(mWidth));
			baseStep.show3 = EditorGUILayout.Toggle("Active/Inactive", baseStep.show3, GUILayout.Width(mWidth));
			baseStep.nextFail = EditorGUILayout.IntField("Next step fail", baseStep.nextFail, GUILayout.Width(mWidth));
			EditorGUILayout.Separator();
		}
	}
	...
	public static void RegenerateStep(int index, GameEvent gameEvent, RegenerateStep baseStep)
	{
		...
		if(baseStep.fold)
		{
			...
			baseStep.show3 = EditorGUILayout.Toggle("Revive Dead", baseStep.show3, GUILayout.Width(mWidth));
