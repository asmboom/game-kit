...
public class BattleEndTab : EditorTab
{
	...
	public void ShowTab()
	{
		...
		if(fold3)
		{
			...
			DataHolder.BattleEnd().wholePartyExp = EditorGUILayout.Toggle("Whole party exp.", 
					DataHolder.BattleEnd().wholePartyExp, GUILayout.Width(pw.mWidth));
			DataHolder.BattleEnd().deadExp = EditorGUILayout.Toggle("Dead receive exp.", 
					DataHolder.BattleEnd().deadExp, GUILayout.Width(pw.mWidth));
