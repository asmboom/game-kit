	...
	*/
	...
	void OnMouseUp()
	{
		if(EventStartType.INTERACT.Equals(this.startType) &&
			this.CheckVariables() && this.gameObject.activeInHierarchy && DataHolder.GameSettings().IsMouseAllowed())
		{
	...
	public override bool Interact()
	{
		...
		if(EventStartType.INTERACT.Equals(this.startType) && GameHandler.IsControlField() &&
			this.CheckVariables() && this.gameObject.activeInHierarchy)
		{
			StartCoroutine(StartBattle());
			val = true;
		}
	...
	public override bool DropInteract(ChoiceContent drop)
	{
		...
		if(EventStartType.DROP.Equals(this.startType) && GameHandler.IsControlField() &&
			this.CheckVariables() && this.gameObject.activeInHierarchy && this.CheckDrop(drop))
		{
			StartCoroutine(StartBattle());
			val = true;
		}
	...
	public IEnumerator StartBattle()
	{
		if(!this.battleRunning)
		{
			...
			for(int i=0; i<this.enemy.Length; i++)
			{
				...
				{
					if(!this.enemyObject[i].activeInHierarchy) this.enemyObject[i].SetActive(true);
