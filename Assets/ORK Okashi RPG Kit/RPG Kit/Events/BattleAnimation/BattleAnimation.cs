...
public class BattleAnimation : BaseEvent
{
	...
	public AnimationStep TypeToStep(BattleAnimationType t)
	{
		...
		// status steps
		else if(BattleAnimationType.CHECK_STATUS_EFFECT.Equals(t)) s = new CheckStatusEffectAStep(t);
		//  item steps
	...
	*/
	public Hashtable GetEventData()
	{
		...
		val.Add("returnlooks", this.returnLooks.ToString());
	...
	public void LoadEventData(Hashtable val)
	{
		...
		if(val.ContainsKey("returntobasecampos"))
		{
			this.returnToBaseCamPos = bool.Parse((string)val["returntobasecampos"]);
		}
		if(val.ContainsKey("returnlooks"))
		{
			this.returnLooks = bool.Parse((string)val["returnlooks"]);
		}
