				...
				if(entry[XMLHandler.NODE_NAME] as string == SaveHandler.SAVEGAMES)
				{
					if(entry.ContainsKey(XMLHandler.NODES))
					{
						...
						foreach(Hashtable ht in subs)
						{
							...
							{
								string sceneName = DataHolder.LoadSaveHUD().saveSceneName;
								int spawnID = DataHolder.LoadSaveHUD().saveSpawnID;
								
								if(ht.ContainsKey("scene"))
								{
									sceneName = (string)ht["scene"];
									spawnID = -1;
								}
								...
								if(ht.ContainsKey(XMLHandler.NODES))
								{
									...
									foreach(Hashtable ht2 in s)
									{
										...
										{
											sceneName = (string)ht2[XMLHandler.CONTENT];
											spawnID = -1;
										}
										// already load variables for event checks
								...
								SaveHandler.LoadScene(sceneName, ht, spawnID);
								return;
							}
						}
					}
