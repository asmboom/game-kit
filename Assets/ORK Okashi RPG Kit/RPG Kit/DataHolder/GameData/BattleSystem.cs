...
public class BattleSystem
{
	...
	public bool deathActionPerforming = false;
	...
	*/
	...
	============================================================================
	*/
	public void DeathActionFinished()
	{
		this.deathActionPerforming = false;
		this.GetNextAction();
	}
	...
	private IEnumerator GetNextAction2()
	{
		...
		if(!this.battleEnd)
		{
			// perform death actions first
			if(this.IsTurnBased() && !this.activeCommand && 
				this.actionStack.Count > 0 && ((BattleAction)this.actionStack[0]).IsDeath())
			{
				this.deathActionPerforming = true;
				BattleAction action = this.actionStack[0] as BattleAction;
				this.actionStack.RemoveAt(0);
				action.PerformAction();
			}
			// choose action
			else if(this.orderIndex < this.actionOrder.Length)
			{
				bool added = false;
			...
			for(int i=0; i<e.status.Length; i++)
			{
				if(e.status[i].IsExperience())
				{
					...
					if(exp != 0)
					{
						...
						{
							if(DataHolder.BattleEnd().wholePartyExp)
							{
								exp /= GameHandler.Party().GetPartySize();
							}
							else
							{
	...
	*/
	...
	public void VictoryGains()
	{
		...
		foreach(DictionaryEntry entry in this.expGains)
		{
			...
			for(int i=0; i<party.Length; i++)
			{
				if(DataHolder.BattleEnd().deadExp || !party[i].isDead)
				{
