...
public class DialoguePositionData : BaseData
{
	...
	public void LoadData()
	{
					...
					if(entry.ContainsKey(XMLHandler.NODES))
					{
						...
						foreach(Hashtable val in subs)
						{
							if(val[XMLHandler.NODE_NAME] as string == DialoguePositionData.DIALOGUEPOSITION)
							{
								...
								position[i].lineSpacing = float.Parse((string)val["linespacing"]);
								...
								if(val.ContainsKey("columnspacing")) position[i].columnSpacing = float.Parse((string)val["columnspacing"]);
								...
								foreach(Hashtable ht in s)
								{
									...
									else if(ht[XMLHandler.NODE_NAME] as string == DialoguePositionData.BOXBOUNDS)
									{
										...
										if(ht.ContainsKey("boxanchor"))
										{
											position[i].boxAnchor = (TextAnchor)System.Enum.Parse(typeof(TextAnchor), (string)ht["boxanchor"]);
										}
									}
		...
		{
			this.AddDialoguePos("Default Position");
		}
	}
	...
	public void SaveData()
	{
		...
		if(name != null)
		{
			for(int i=0; i<name.Length; i++)
			{
				...
				n.Add("boxanchor", position[i].boxAnchor.ToString());
				s.Add(n);
	...
	public DialoguePosition GetCopy(int index)
	{
		...
		p.boxAnchor = position[index].boxAnchor;
