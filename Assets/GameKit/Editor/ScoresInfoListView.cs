...
namespace Beetle23
{
    public class ScoresInfoListView
    {
        public ScoresInfoListView(World world)
        {
        ...
        public void UpdateDisplayItem(World world)
        {
            _currentDisplayedWorld = world;
            if (world != null)
            {
                _listAdaptor = new GenericClassListAdaptor<Score>(world.Scores, 18,
                    CreateScore, DrawScoreElement);
        ...
        public Score DrawScoreElement(Rect position, Score element, int index)
        {
        ...
        public void UpdateItemIndices(bool needWarning)
        {
            if (_listAdaptor != null)
            {
                ...
                {
                    if (_listAdaptor[i].RelatedVirtualItem == null && VirtualItemsEditUtil.DisplayedItemIDs.Length > 0)
                    {
                        VirtualItem item = GameKit.Config.GetVirtualItemByID(VirtualItemsEditUtil.DisplayedItemIDs[0]);
                        if (item != null && needWarning)
                        {
                            Debug.LogWarning("[" + _currentDisplayedWorld.ID + "]'s [" + (i + 1) + 
                                "] item is null, correct it with default item [" + item.ID + "].");
                        }
                        _listAdaptor[i].RelatedVirtualItemID = item.ID;
                    }
                    _itemIndices.Add(_listAdaptor[i].RelatedVirtualItem != null ?
                        VirtualItemsEditUtil.GetItemIndexById(_listAdaptor[i].RelatedVirtualItem.ID) : 0);
                }
            }
        }
        ...
        private World _currentDisplayedWorld;
        private ReorderableListControl _listControl;
        private GenericClassListAdaptor<Score> _listAdaptor;
