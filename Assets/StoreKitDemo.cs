...
public class StoreKitDemo : MonoBehaviour
{
    private void Awake()
    {
        StoreKit.Init(new DemoStoreKitFactory());

        _coin = StoreKit.Config.GetItemByID("cn.ftang.coin");
        _gem = StoreKit.Config.GetItemByID("cn.ftang.gem");
        _purchasableItems = new List<VirtualItem>();
        for (int i = 0; i < StoreKit.Config.Items.Count; i++)
        {
            VirtualItem item = StoreKit.Config.Items[i];
        ...
        Market.Instance.StartProductListRequest();
    }
    ...
    private void FirstTimeLaunch()
    {
        ...
        StoreKit.Config.GetItemByID("cn.ftang.wuzheng").Give(1);
        StoreKit.Config.GetItemByID("cn.ftang.wuzheng").Equip();
        _coin.Give(4000);
        _gem.Give(5);
    ...
    {
        return PlayerPrefsStoreStorage.GetBool("already_launched", false);
    }

    private void SetLaunched()
    {
        PlayerPrefsStoreStorage.SetBool("already_launched", true);
    }
